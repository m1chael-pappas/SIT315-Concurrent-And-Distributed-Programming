...
// Extra implementation: parallel quicksort using std::thread directly.
//
// Same partition code as the other two programs. The difference from the
// OpenMP version is that there is no work-stealing runtime underneath. I spawn
// a real OS thread for the left half and keep the right half on the current
// thread, down to a fixed depth. Depth d allows at most 2^d threads at once,
// so I set d = ceil(log2(threads)).
//
...
// ========================== STD::THREAD QUICKSORT ===========================

// depth counts how many more times I am allowed to fork. Each fork spawns one
// new thread for the left half while the caller takes the right half, so the
// live thread count tops out at 2^depth. Past that, or once a subarray drops
// below size_cutoff, it falls back to plain recursion because a thread costs
// more to create than a small sort costs to run.
static void quicksort(int32_t* a, int64_t lo, int64_t hi, Pivot rule,
                      int depth, int64_t size_cutoff) {
    if (lo >= hi) return;
    ...
    if (depth > 0 && (hi - lo + 1) > size_cutoff) {
        std::thread left(quicksort, a, lo, p, rule, depth - 1, size_cutoff);
        quicksort(a, p + 1, hi, rule, depth - 1, size_cutoff);
        left.join();   // the join is the only synchronisation in this file
    } else {
        quicksort(a, lo, p, rule, depth, size_cutoff);
        quicksort(a, p + 1, hi, rule, depth, size_cutoff);
    }
}
...
int main(int argc, char** argv) {
    ...
    int64_t size_cutoff = arg_ll(argc, argv, "--size-cutoff", 100000);

    int depth = 0;
    while ((1 << depth) < threads) ++depth;

    std::printf("program      quicksort_parallel_threads\n");
    std::vector<int32_t> original = get_input(argc, argv);
    uint64_t want = checksum(original);

    std::printf("elements     %zu\n", original.size());
    std::printf("bytes        %zu\n", original.size() * 4);
    std::printf("threads      %d (fork depth %d)\n", threads, depth);
    std::printf("pivot        %s\n", pivot_name(rule));
    std::printf("size-cutoff  %lld\n", static_cast<long long>(size_cutoff));
    ...
    for (int r = 0; r < runs; ++r) {
        ...
        t.start();
        if (!a.empty())
            quicksort(a.data(), 0, static_cast<int64_t>(a.size()) - 1, rule, depth, size_cutoff);
        double ms = t.stop_ms();
